...
namespace glControlKit
{
	...
	{
		public static Texture2D CreateTexture2D(Color color, int width = 64, int height = 64) {
		...
		public static Texture2D CreateTexture2D(Bitmap bmp, out Color[] pixels) {
			int id = GL.GenTexture();
			int i = 0;
			pixels = new Color[bmp.Width * bmp.Height];
			for (int x = 0; x < bmp.Width; x++) {
				for (int y = 0; y < bmp.Height; y++) {
					pixels[i++] = bmp.GetPixel(x, y);
				}
			}
			GL.BindTexture(TextureTarget.Texture2D, id);
			BitmapData data = bmp.LockBits(new Rectangle(0, 0, bmp.Width, bmp.Height), ImageLockMode.ReadOnly,
